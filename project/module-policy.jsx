// module-policy.jsx — bi-directional editor: plain text ↔ YAML
// Novel metaphor: text and YAML are "twin shadows" of the same policy.
// Editing the text "rotates" matching tokens in the YAML in real time.

function PolicyEditor({ selectedId, setSelected, setRoute }) {
  const { STATUS_META } = window.LR_DATA;
  const { policies: POLICIES } = window.usePolicies();
  const toast = window.useToast();
  const initial = POLICIES.find(p => p.id === selectedId) || POLICIES[0];
  const [text, setText] = React.useState(initial.text);
  const [yaml, setYaml] = React.useState(initial.yaml);
  const [active, setActive] = React.useState(null); // hovered token id

  // Sync editor state when the selected policy changes
  React.useEffect(() => {
    setText(initial.text);
    setYaml(initial.yaml);
  }, [initial.id]);

  // Detect drift: live hash compare (cheap stand-in)
  const drift = React.useMemo(() => {
    return text !== initial.text || yaml !== initial.yaml;
  }, [text, yaml, initial]);

  // Mappings: simple regex-driven bridges between text tokens and YAML keys.
  // Each mapping derives a value from the text and writes it into the YAML on change.
  const mappings = React.useMemo(() => buildMappings(initial.id), [initial.id]);

  // When text changes, derive YAML rewrites for any matched token
  ...
  // Highlight a token in both panes when hovered
  ...
  return (
    <div className="lr-page" style={{ paddingBottom: 30 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 18, marginBottom: 16, padding: "10px 14px", border: "1px solid var(--line-1)", borderRadius: 8, background: "var(--bg-1)" }}>
        ...
        <MetaItem k="Last Anchor" v={<span className="mono dim">{window.relTime(initial.lastAnchor)}</span>} />
      </div>
...
// Each entry knows how to extract a value from one side and write it into the
// other. For prototype purposes, we cover a couple of high-signal tokens per
// policy — enough to show real-time round-trip in the editor.
...
function renderHighlighted(str, mappings, activeId, side) {
  // Build a list of {start, end, id} from each mapping's regex
  const hits = [];
  for (const m of mappings) {
  ...
  // Merge into a sequence of plain + token chunks
  const out = [];
  let cur = 0;
  for (const h of hits) {
    if (h.start < cur) continue; // skip overlapping
