// module-brain.jsx — Governance Brain: policies as a constellation
// Novel metaphor: each policy is a body in a force-field. Risk weight = mass.
// Tethers connect policies to their technical hooks. Collisions = conflicts.
...
function Brain() {
  ...
  // Layout: deterministic pseudo-random positions seeded by policy id
  ...
  return (
    <div className="lr-card" style={{ padding: 16 }}>
      ...
      <button className="lr-btn primary" style={{ marginTop: 14, width: "100%", justifyContent: "center" }}
        onClick={() => window.__navigate?.("editor", hovered.id)}>Open in Editor →</button>
    </div>
  );
}
...
// Pseudo-random but deterministic layout per id
function hashSeed(s) {
  let h = 0;
...
function layoutConstellation(policies, bump = 0) {
  ...
  return policies.map((p, i) => {
    ...
    // Cluster by category — assign an angle band
...
function deriveEdges(nodes) {
  const edges = [];
  // Conflict edges (defined in policy.conflict)
  for (const n of nodes) {
    if (n.conflict?.with) edges.push({ from: n.id, to: n.conflict.with, kind: "conflict" });
  }
  // Sympathetic edges: same primary category
  const byCat = {};
  for (const n of nodes) {
