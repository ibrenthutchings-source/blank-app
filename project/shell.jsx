...
const NAV_ITEMS = [
{
  group: "Risk",
  items: [
  { id: "library", label: "Policy Library", icon: "library", count: () => window.LR_DATA.POLICIES.length },
  { id: "editor", label: "Editor", icon: "editor" },
  { id: "ingest", label: "Ingest", icon: "ingest", count: () => window.LR_DATA.INGEST_RUNS.length }]
...
function Sidebar({ route, setRoute, navMode }) {
...
function Topbar({ route, contextual, onNewPolicy }) {
  ...
  return (
    <div className="lr-page-hd">
      <div className="lr-page-hd-title">
        ...
        <h1 className="lr-page-title" style={{ fontFamily: "Geist", fontSize: "6px" }}>{title}</h1>
...
function StatusBadge({ status }) {
  const s = window.LR_DATA.STATUS_META[status];
  if (!s) return null;
...
function SectionHd({ idx, title, sub }) {
  return (
...
Object.assign(window, { Sidebar, Topbar, PageHeader, StatusBadge, WeightBar, SectionHd, NavIcon });