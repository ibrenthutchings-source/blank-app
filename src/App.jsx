...
import { useTweaks, TweaksPanel, TweakSection, TweakColor, TweakRadio, TweakToggle, TweakSlider, TweakButton } from './components/TweaksPanel'
import { ToastProvider, useToast, PolicyProvider, usePolicies } from './contexts'
import { Sidebar, Topbar } from './components/Shell'
import { NewPolicyModal, VerifyChainOverlay } from './components/Interactions'
import { Library } from './modules/Library'
import { PolicyEditor } from './modules/PolicyEditor'
...
function AppInner() {
  ...
  const Page = {
    library:   <Library setRoute={setRoute} setSelected={setSelected} onNewPolicy={() => setNewPolicyOpen(true)} />,
    editor:    <PolicyEditor selectedId={selected} setSelected={setSelected} setRoute={setRoute} />,
    ingest:    <Ingest />,
    brain:     <Brain onNavigate={(r, id) => { setRoute(r); if (id) setSelected(id); }} />,
    ledger:    <Ledger onVerifyChain={() => setVerifying(true)} />,
    pipelines: <Pipelines />,
    settings:  <Settings />,
  }[route];
  ...
  return (
    <>
      ...
      <div className="lr-app" data-nav={t.navMode} data-density={t.density}>
        <Sidebar route={route} setRoute={setRoute} navMode={t.navMode} policies={policies} ingestRuns={INGEST_RUNS} />
